...
create table User(
	...
	userType int NOT NULL default 0 -- Utilizzato per identificare il tipo di utente, solo il venditore avrà una valutazione
									-- 0 -> normalUser
									-- 1 -> seller
									-- 2 -> amministratore
);

CREATE INDEX idUserIndex ON User (id,userType) USING HASH;
...
create table Negozio(
	...
	IdI int NOT NULL,
	PRIMARY KEY(id),
	FOREIGN KEY idVenditore REFERENCES User(id) ON DELETE NO ACTION,
...
create table Oggetto(
	id varchar(50) NOT NULL -- Codifico il nome in md5 o sh1 in modo da avere la chiave univoca per questo oggetto
...
create table Ordine(
	...
	PRIMARY KEY(idOrdine,idOggetto,idNegozio,idUtente),
	FOREIGN KEY (idOggetto) REFERENCES Oggetto(id) ON DELETE NO ACTION,
	FOREIGN KEY (idNegozio) REFERENCES Oggetto(idNegozio) ON DELETE NO ACTION,
	FOREIGN KEY (idUtente) REFERENCES User(id) ON DELETE CASCADE 	-- Se viene eliminato un utente che aveva fatto questo ordine
...
create table Assistenza(
	...
	PRIMARY KEY(id),
	FOREIGN KEY (idUtente) REFERENCES User(id) ON DELETE NO ACTION,				-- Mantengo i dati sull'assistenza anche dopo la cancellazione di un singolo utente
																				-- ipotesi:
	...
	FOREIGN KEY (idVenditore) REFERENCES User(id) ON DELETE NO ACTION,
	FOREIGN KEY (idAmministratore) REFERENCES User(id) ON DELETE NO ACTION,
	FOREIGN KEY (idOrdine) REFERENCES Ordine(id) ON DELETE NO ACTION
);
...
create table RecensioneOggetto(
	...
	FOREIGN KEY (idUtente) REFERENCES User(id) ON DELETE NO ACTION
);
...
create table RecensioneVenditore(
	...
	utilita int default NULL,
	PRIMARY KEY(id),
	FOREIGN KEY (idVenditore) REFERENCES Venditore(id) ON DELETE CASCADE
	FOREIGN KEY (idUtente) REFERENCES User(id) ON DELETE NO ACTION
);
